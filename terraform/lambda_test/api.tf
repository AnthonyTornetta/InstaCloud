...
resource "aws_api_gateway_resource" "api_resource_{function_name}" {
...
resource "aws_api_gateway_method" "api_method_{function_name}" {
  rest_api_id   = aws_api_gateway_rest_api.api_gateway.id
  resource_id   = aws_api_gateway_resource.api_resource_{function_name}.id
...
resource "aws_api_gateway_deployment" "api_deployment_{function_name}" {
  depends_on = [aws_api_gateway_integration.lambda_integration_{function_name}]
